...
COMMENT ON COLUMN sosl_run_queues.run_state IS 'Holds the run state: 0 Waiting, 1 Enqueued, 2 Started, 3 Running, 4 Finished, -1 Error. To rerun a job, set run_state to 1. Will not be accepted if executor is not active and reviewed. Script dependencies are not checked. Always 0 on insert, managed by trigger';
COMMENT ON COLUMN sosl_run_queues.executor_id IS 'The valid executor id as returned from API (NUMBER). No updates allowed, surpressed by trigger.';
COMMENT ON COLUMN sosl_run_queues.ext_script_id IS 'The (external) identifier for the current script as returned from API (VARCHAR2). No updates allowed, surpressed by trigger.';
COMMENT ON COLUMN sosl_run_queues.script_file IS 'The script file name including (relative) path from API (VARCHAR2). No updates allowed, surpressed by trigger.';
...
ALTER TABLE sosl_run_queues
  ADD CONSTRAINT sosl_run_queues_executor_id_fk
