...
;
-- description
COMMENT ON TABLE sosl_script IS 'Holds the script file names that should be executed by SOSL. Used for tests and simple batch script setups. No logic control in triggers apart from insert and update dates and users. Will use the alias scrt.';
COMMENT ON COLUMN sosl_script.script_id IS 'The generated unique id of the script file.';
COMMENT ON COLUMN sosl_script.executor_id IS 'The optional related executor id of the script file. If defined, must match an existing executor. If not defined, script is ignored.';
...
COMMENT ON COLUMN sosl_script.run_order IS 'The order in which the script file should be executed. Same number means in parallel. Higher order numbers wait for scripts with lower order numbers to complete. Must be greater than 0.';
COMMENT ON COLUMN sosl_script.run_state IS 'Holds the run state: 0 Waiting, 1 Enqueued, 2 Started, 3 Running, 4 Finished, -1 Error. To rerun a job, set run_state to 1. Will not be accepted if executor is not active and reviewed. Script dependencies are not checked. Can only be 0 or -1 on insert, managed by trigger';
COMMENT ON COLUMN sosl_script.run_group IS 'A minimalistic group ID provided as is. Mixed case delivers mixed results. Up to installer to manage specific groups. Allows grouping of scripts.';
...
-- constraints
ALTER TABLE sosl_script
  ADD CONSTRAINT sosl_script_chk_run_state
