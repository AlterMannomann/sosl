...
COMMENT ON COLUMN sosl_executor.mail_sender IS 'The mail address of the mail sender to use for mail. Must be a valid email address. If not set then mail will not get activated.';
COMMENT ON COLUMN sosl_executor.mail_recipients IS 'The list of valid recipient email addresses separated by a semicolon. Use mail groups and keep this list short. Limited to 1024 char. At least one recipient must be given, if mail is activated.';
