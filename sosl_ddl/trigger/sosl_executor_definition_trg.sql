...
CREATE OR REPLACE TRIGGER sosl_executor_definition_ins_trg
  BEFORE INSERT ON sosl_executor_definition
  FOR EACH ROW
DECLARE
  l_self_log_category sosl_server_log.log_category%TYPE   := 'SOSL_EXECUTOR_DEFINITION';
  l_self_caller       sosl_server_log.caller%TYPE         := 'sosl_executor_definition_ins_trg';
BEGIN
  :NEW.created            := SYSDATE;
  ...
  THEN
    sosl_log.minimal_error_log( l_self_caller
                              ...
                              , 'Wrong database user for SOSL_EXECUTOR_DEFINITION table issued by DB user: ' || SYS_CONTEXT('USERENV', 'SESSION_USER') || ' OS user: ' || SYS_CONTEXT('USERENV', 'OS_USER')
                              )
    ;
  ...
  THEN
    sosl_log.minimal_error_log( l_self_caller
                              ...
                              , 'Wrong function owner for SOSL_EXECUTOR_DEFINITION table issued by DB user: ' || SYS_CONTEXT('USERENV', 'SESSION_USER') || ' OS user: ' || SYS_CONTEXT('USERENV', 'OS_USER')
                              )
    ;
  ...
  THEN
    sosl_log.minimal_error_log( l_self_caller
                              ...
                              , 'Wrong function has_scripts for SOSL_EXECUTOR_DEFINITION table issued by DB user: ' || SYS_CONTEXT('USERENV', 'SESSION_USER') || ' OS user: ' || SYS_CONTEXT('USERENV', 'OS_USER')
                              )
    ;
  ...
  THEN
    sosl_log.minimal_error_log( l_self_caller
                              ...
                              , 'Wrong function get_next_script for SOSL_EXECUTOR_DEFINITION table issued by DB user: ' || SYS_CONTEXT('USERENV', 'SESSION_USER') || ' OS user: ' || SYS_CONTEXT('USERENV', 'OS_USER')
                              )
    ;
  ...
  THEN
    sosl_log.minimal_error_log( l_self_caller
                              ...
                              , 'Wrong function set_script_status for SOSL_EXECUTOR_DEFINITION table issued by DB user: ' || SYS_CONTEXT('USERENV', 'SESSION_USER') || ' OS user: ' || SYS_CONTEXT('USERENV', 'OS_USER')
                              )
    ;
  ...
  THEN
    ...
    THEN
      sosl_log.minimal_error_log( l_self_caller
                                ...
                                , 'Wrong function send_db_mail for SOSL_EXECUTOR_DEFINITION table issued by DB user: ' || SYS_CONTEXT('USERENV', 'SESSION_USER') || ' OS user: ' || SYS_CONTEXT('USERENV', 'OS_USER')
                                )
      ;
...
END;
/
CREATE OR REPLACE TRIGGER sosl_executor_definition_upd_trg
  BEFORE UPDATE ON sosl_executor_definition
  FOR EACH ROW
DECLARE
  l_self_log_category sosl_server_log.log_category%TYPE   := 'SOSL_EXECUTOR_DEFINITION';
  l_self_caller       sosl_server_log.caller%TYPE         := 'sosl_executor_definition_upd_trg';
BEGIN
  :NEW.updated        := SYSDATE;
  :NEW.updated_by     := SYS_CONTEXT('USERENV', 'SESSION_USER');
  :NEW.updated_by_os  := SYS_CONTEXT('USERENV', 'OS_USER');
  -- no overwrite for this values, log changes
  sosl_log.log_column_change(:OLD.created, :NEW.created, 'SOSL_EXECUTOR_DEFINITION.CREATED', l_self_caller);
  :NEW.created := :OLD.created;
  sosl_log.log_column_change(:OLD.created_by, :NEW.created_by, 'SOSL_EXECUTOR_DEFINITION.CREATED_BY', l_self_caller);
  :NEW.created_by := :OLD.created_by;
  sosl_log.log_column_change(:OLD.created_by_os, :NEW.created_by_os, 'SOSL_EXECUTOR_DEFINITION.CREATED_BY_OS', l_self_caller);
  :NEW.created_by_os := :OLD.created_by_os;
  sosl_log.log_column_change(:OLD.function_owner, :NEW.function_owner, 'SOSL_EXECUTOR_DEFINITION.FUNCTION_OWNER', l_self_caller);
  :NEW.function_owner := :OLD.function_owner;
  sosl_log.log_column_change(:OLD.db_user, :NEW.db_user, 'SOSL_EXECUTOR_DEFINITION.DB_USER', l_self_caller);
  :NEW.db_user := :OLD.db_user;
  -- prepare possibly modified values
  sosl_log.log_column_change(:OLD.executor_active, :NEW.executor_active, 'SOSL_EXECUTOR_DEFINITION.EXECUTOR_ACTIVE', l_self_caller, FALSE);
  sosl_log.log_column_change(:OLD.executor_reviewed, :NEW.executor_reviewed, 'SOSL_EXECUTOR_DEFINITION.EXECUTOR_REVIEWED', l_self_caller, FALSE);
  sosl_log.log_column_change(:OLD.use_mail, :NEW.use_mail, 'SOSL_EXECUTOR_DEFINITION.USE_MAIL', l_self_caller, FALSE);
  sosl_log.log_column_change(:OLD.fn_has_scripts, :NEW.fn_has_scripts, 'SOSL_EXECUTOR_DEFINITION.FN_HAS_SCRIPTS', l_self_caller, FALSE);
  :NEW.fn_has_scripts := UPPER(:NEW.fn_has_scripts);
  sosl_log.log_column_change(:OLD.fn_get_next_script, :NEW.fn_get_next_script, 'SOSL_EXECUTOR_DEFINITION.FN_GET_NEXT_SCRIPT', l_self_caller, FALSE);
  :NEW.fn_get_next_script := UPPER(:NEW.fn_get_next_script);
  sosl_log.log_column_change(:OLD.fn_set_script_status, :NEW.fn_set_script_status, 'SOSL_EXECUTOR_DEFINITION.FN_SET_SCRIPT_STATUS', l_self_caller, FALSE);
  :NEW.fn_set_script_status := UPPER(:NEW.fn_set_script_status);
  sosl_log.log_column_change(:OLD.fn_send_db_mail, :NEW.fn_send_db_mail, 'SOSL_EXECUTOR_DEFINITION.FN_SEND_DB_MAIL', l_self_caller, FALSE);
  :NEW.fn_send_db_mail := UPPER(:NEW.fn_send_db_mail);
  ...
  THEN
    ...
    THEN
      ...
      sosl_log.minimal_error_log( l_self_caller
                                ...
                                , 'Failed granting necessary roles for SOSL_EXECUTOR_DEFINITION table issued by DB user: ' || SYS_CONTEXT('USERENV', 'SESSION_USER') || ' OS user: ' || SYS_CONTEXT('USERENV', 'OS_USER')
                                )
      ;
