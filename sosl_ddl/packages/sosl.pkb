...
AS
  ...
  IS
    ...
    l_caller      sosl_server_log.caller%TYPE       := 'sosl.has_scripts queue and executor';
  ...
  BEGIN
    l_return      := -1;
    l_success_cnt := 0;
    sosl_log.full_log( p_message => 'sosl.has_scripts called'
    ...
    THEN
      ...
      LOOP
        l_tmp_cnt := sosl.has_scripts_for_fn(rec.fn_has_scripts);
        IF l_tmp_cnt > 0
        THEN
      ...
      l_tmp_cnt := sosl.has_scripts(l_queue_table);
      IF l_tmp_cnt > 0
      THEN
        l_success_cnt := l_success_cnt +1;
        l_return      := l_return + l_tmp_cnt;
      ...
      IF l_success_cnt <= 0
      THEN
        sosl_log.full_log( p_message => 'sosl.has_scripts did not find any valid executor has_scripts functions and messages in the queue. Return 0 scripts waiting.'
    ...
    ELSE
      ...
      sosl_log.full_log( p_message => 'sosl.has_scripts called without valid executors defined. Return 0 scripts waiting.'
  ...
  IS
    ...
    l_category      sosl_server_log.log_category%TYPE   := 'HAS_SCRIPTS';
    l_caller        sosl_server_log.caller%TYPE         := 'sosl.has_scripts executor';
  BEGIN
    l_return := -1;
    sosl_log.full_log( p_message => 'sosl.has_scripts executer check p_identifier: ' || p_identifier
                     , p_log_type => sosl_sys.INFO_TYPE
                     , p_log_category => l_category
                     , p_caller => l_caller
                     )
    ...
    ELSE
      ...
      l_return := sosl.has_scripts_for_fn(l_fn_call);
    END IF;
    RETURN l_return;
  EXCEPTION
    WHEN OTHERS THEN
      -- log the error instead of RAISE
      sosl_log.full_log( p_message => 'Unhandled exception in sosl.has_scripts for executor function: ' || SQLERRM
                       , p_log_type => sosl_sys.FATAL_TYPE
                       , p_log_category => l_category
                       , p_caller => l_caller
                       )
  ...
  IS
    ...
    l_return      NUMBER;
    l_category    sosl_server_log.log_category%TYPE := 'HAS_SCRIPTS';
    l_caller      sosl_server_log.caller%TYPE       := 'sosl.has_scripts queue';
  BEGIN
    l_return := -1;
    sosl_log.full_log( p_message => 'sosl.has_scripts queue check p_identifier: ' || p_identifier
                     , p_log_type => sosl_sys.INFO_TYPE
                     , p_log_category => l_category
                     , p_caller => l_caller
                     )
    ...
    ELSE
      ...
      EXCEPTION
        WHEN OTHERS THEN
          sosl_log.full_log( p_message => 'Unhandled exception in sosl.has_scripts for queue function checking queue table: ' || SQLERRM
                           , p_log_type => sosl_sys.FATAL_TYPE
                           , p_log_category => l_category
                           , p_caller => l_caller
                           )
  ...
  EXCEPTION
    WHEN OTHERS THEN
      -- log the error instead of RAISE
      sosl_log.full_log( p_message => 'Unhandled exception in sosl.has_scripts for queue function: ' || SQLERRM
                       , p_log_type => sosl_sys.FATAL_TYPE
                       , p_log_category => l_category
                       , p_caller => l_caller
                       )
  ...
  IS
    ...
    l_fn_call       sosl_executor.fn_has_scripts%TYPE;
    l_category      sosl_server_log.log_category%TYPE   := 'HAS_SCRIPTS';
    l_caller        sosl_server_log.caller%TYPE         := 'sosl.has_scripts_for_fn';
  ...
  BEGIN
    l_return  := -1;
    sosl_log.full_log( p_message => 'sosl.has_scripts_for_fn defined function check p_fn_has_scripts: ' || p_fn_has_scripts
                     , p_log_type => sosl_sys.INFO_TYPE
                     , p_log_category => l_category
                     , p_caller => l_caller
                     )
    ...
    ELSE
      ...
      THEN
        ...
        EXCEPTION
          WHEN OTHERS THEN
            sosl_log.full_log( p_message => 'Unhandled exception in sosl.has_scripts_for_fn for calling defined function: ' || p_fn_has_scripts || ' Error: ' || SQLERRM
                             , p_log_type => sosl_sys.FATAL_TYPE
                             , p_log_category => l_category
                             , p_caller => l_caller
                             )
  ...
  EXCEPTION
    WHEN OTHERS THEN
      -- log the error instead of RAISE
      sosl_log.full_log( p_message => 'Unhandled exception in sosl.has_scripts_for_fn function: ' || SQLERRM
                       , p_log_type => sosl_sys.FATAL_TYPE
                       , p_log_category => l_category
                       , p_caller => l_caller
                       )
